/**
 * eis.cpp - Gyro-first EIS (Quaternion)
 *
 * Main path:
 *   - Gyro 3-axis integration (quaternion)
 *   - Virtual camera smoothing (slerp)
 *   - Correction rotation -> H = K * R_corr * K^-1
 *   - warpPerspective + center crop
 *
 * LK optical flow is kept only for offset sweep/debug comparison.
 *
 * RTSP outputs:
 *   /raw  original
 *   /cam  stabilized
...
static std::atomic<int> g_mode{(int)EisMode::GYRO};
...
static GyroBuffer g_gyro_buffer(IMU_STORE_SIZE);
static GyroSample g_last_gyro_sample;
...
public:
    ...
    bool get_frame(Mat &out, double &out_time_ms, TsSource &out_src,
        ...
        Mat frame((int)cfg.size.height, (int)cfg.size.width, CV_8UC4, data, stride);
        out = frame.clone();
...
static void capture_loop() {
    ...
    auto pull_frame_gst = [&](CapturedFrame& out) -> bool {
        ...
        out.time_ms = t_arrival;

        TsSourcePref cur_pref = (TsSourcePref)g_ts_pref.load();
        bool used_pref = true;

        if (cur_pref == TsSourcePref::AUTO) {
            if (have_sensor) { out.src = TsSource::SENSOR; out.time_ms = t_sensor; }
            else if (have_pts) { out.src = TsSource::PTS; out.time_ms = t_pts; }
        } else if (cur_pref == TsSourcePref::SENSOR) {
            if (have_sensor) { out.src = TsSource::SENSOR; out.time_ms = t_sensor; }
            else if (have_pts) { out.src = TsSource::PTS; out.time_ms = t_pts; used_pref = false; }
            else used_pref = false;
        } else if (cur_pref == TsSourcePref::PTS) {
            if (have_pts) { out.src = TsSource::PTS; out.time_ms = t_pts; }
            else if (have_sensor) { out.src = TsSource::SENSOR; out.time_ms = t_sensor; used_pref = false; }
            else used_pref = false;
        } else if (cur_pref == TsSourcePref::ARRIVAL) {
            out.src = TsSource::ARRIVAL;
            out.time_ms = t_arrival;
        }
        ...
        if (!used_pref && !ts_pref_warned) {
            fprintf(stderr, "[TS] preferred %s not available -> fallback %s\n",
                    ts_pref_str(cur_pref), ts_source_str(out.src));
            ts_pref_warned = true;
        }

        out.sensor_ts_ns = 0;
        out.exp_us = 0;
        out.index = gst_frame_index++;
