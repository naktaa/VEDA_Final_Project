#include <iostream>
  ...
  static bool isExtraMarkerId(int id)
  {
  ...
  int main(int argc, char** argv)
  {
      ...
      while (true) {
          ...
          // Publish pose for every visible marker in target set (21~25) and extra ids (10~13)
          for (size_t i = 0; i < ids.size(); i++) {
              const int id = ids[i];
              const bool inTargetRange = (id >= targetMarkerMinId && id <= targetMarkerMaxId);
              const bool inExtra = isExtraMarkerId(id);
              if (!inTargetRange && !inExtra) continue;

              cv::Mat R_cam_marker_m;
              cv::Rodrigues(rvecs[i], R_cam_marker_m);
              cv::Matx33d R_cam_marker = toMatx33d(R_cam_marker_m);

              cv::Matx33d R_world_obj;
              cv::Vec3d t_world_obj;
              ...
              if (inTargetRange) {
                  ...
                  const cv::Vec3d t_m_c = -(R_m_c * cfg.t_c_m);

                  const cv::Matx33d R_cam_cube = R_cam_marker * R_m_c;
                  const cv::Vec3d t_cam_cube = (R_cam_marker * t_m_c) + tvecs[i];

                  R_world_obj = R_world_cam * R_cam_cube;
                  t_world_obj = (R_world_cam * t_cam_cube) + t_world_cam;

                  const cv::Vec3d rc_offset(rcOffsetX, rcOffsetY, rcOffsetZ);
                  t_world_obj = t_world_obj + (R_world_obj * rc_offset);
              } else {
                  // Extra marker ids: use marker origin directly (no cube model)
                  R_world_obj = R_world_cam * R_cam_marker;
                  t_world_obj = (R_world_cam * tvecs[i]) + t_world_cam;
              }

              const cv::Vec3d forward = R_world_obj * cv::Vec3d(0.0, 1.0, 0.0);
              const double yaw = normalizeAngle(std::atan2(forward[1], forward[0]));
              const long long ts = nowMs();

              char buf[256];
              std::snprintf(buf, sizeof(buf),
                            "{\"id\":%d,\"x\":%.3f,\"y\":%.3f,\"yaw\":%.6f,\"frame\":\"world\",\"ts_ms\":%lld}",
                            id, t_world_obj[0], t_world_obj[1], yaw, ts);

              std::string topic = mqttTopic + "/" + std::to_string(id);
              if (mqttPublishJson(mosq, topic, buf)) {
                  detectCount++;
                  publishCount++;
              }
          }
