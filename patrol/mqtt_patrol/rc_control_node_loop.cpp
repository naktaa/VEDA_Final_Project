...
auto g_last_pose_log = std::chrono::steady_clock::now();
auto g_last_servo_log = std::chrono::steady_clock::now();
std::atomic<long long> g_last_pose_recv_ms{0};
    ...
    if (!use_mqtt_) {
        {
            ...
            pose_.ts_ms = nowMs();
            pose_.valid = true;
            dr_pose_initialized_ = true;
            last_dr_tick_ = std::chrono::steady_clock::now();
        }
        running_ = true;
        std::cout << "[OK] RC control node started. mqtt=off\n";
        return true;
    }
            ...
            if (goal_input_mode) {
                if (ch == '\r' || ch == '\n') {
                    ...
                    if (iss >> gx >> gy) {
                        ...
                        std::cout << "\n[WARN] invalid goal input: '" << goal_buf << "' (use: x y)\n";
                    }
                    goal_buf.clear();
                    goal_input_mode = false;
                } else if (ch == 127 || ch == 8) {
                    if (!goal_buf.empty()) {
                        goal_buf.pop_back();
                    ...
                    goal_buf.push_back(static_cast<char>(ch));
...
void RcControlNode::onMessage(const struct mosquitto_message* msg) {
    ...
    if (topic == topic_goal_) {
        ...
        if (parseGoalJson(payload, g) && g.frame == "world") {
            ...
            std::cout << "[GOAL] x=" << g.x << " y=" << g.y << "\n";
        } else {
            std::cerr << "[WARN] invalid goal payload: " << payload << "\n";
        }
    } else if (topic == topic_pose_) {
        RcPose p;
        if (parsePoseJson(payload, p)) {
            if (p.frame.empty()) p.frame = "world";
            const auto now = std::chrono::steady_clock::now();
            const long long now_ms = nowMs();
            if (p.ts_ms > 0 && (now_ms - p.ts_ms) > 1500) {
                g_last_pose_recv_ms.store(now_ms, std::memory_order_relaxed);
                return;
            }
            const double kStagnantPosM = 0.10;
            if (have_last_pose_raw_) {
                if (p.ts_ms > 0 && last_pose_raw_.ts_ms > 0 && p.ts_ms < last_pose_raw_.ts_ms) {
                    g_last_pose_recv_ms.store(now_ms, std::memory_order_relaxed);
                    return;
                }
                const double dx = p.x - last_pose_raw_.x;
                const double dy = p.y - last_pose_raw_.y;
                const double dpos = std::hypot(dx, dy);
                const bool stagnant = (dpos < kStagnantPosM);
                if (stagnant) {
                    last_pose_raw_ = p;
                    g_last_pose_recv_ms.store(now_ms, std::memory_order_relaxed);
                    return;
            ...
            pose_ = p;
            last_pose_raw_ = p;
            have_last_pose_raw_ = true;
            dr_pose_initialized_ = true;
            last_dr_tick_ = std::chrono::steady_clock::now();
            g_last_pose_recv_ms.store(now_ms, std::memory_order_relaxed);
            if (now - g_last_pose_log >= std::chrono::seconds(1)) {
                static RcPose last_pose_logged{};
                static bool have_last_pose_logged = false;
                const double dxl = p.x - last_pose_logged.x;
                const double dyl = p.y - last_pose_logged.y;
                const double dpl = std::hypot(dxl, dyl);
                if (!have_last_pose_logged || dpl >= 1e-6) {
                    std::cout << "[POSE] x=" << p.x << " y=" << p.y << " yaw=" << p.yaw << "\n";
                    last_pose_logged = p;
                    have_last_pose_logged = true;
                }
                g_last_pose_log = now;
            }
        } else {
            std::cerr << "[WARN] invalid/unsupported pose payload: " << payload << "\n";
        }
...
void RcControlNode::controlStep() {
    ...
    static ReverseWiggleState reverse_wiggle_state = ReverseWiggleState::NONE;
    static auto reverse_wiggle_until = std::chrono::steady_clock::now();

    RcGoal goal;
    RcPose pose;
    RcSafety safety;
    bool reached_hold = false;
    std::chrono::steady_clock::time_point last_dr_tick_snapshot = std::chrono::steady_clock::now();
    bool dr_initialized = false;
    static auto last_pose_stale_log = std::chrono::steady_clock::now() - std::chrono::seconds(1);
    {
        std::lock_guard<std::mutex> lk(data_mtx_);
        goal = goal_;
        pose = pose_;
        ...
        dr_initialized = dr_pose_initialized_;
        last_dr_tick_snapshot = last_dr_tick_;
    }

    if (use_mqtt_) {
        const auto now_ms = nowMs();
        const long long last_ms = g_last_pose_recv_ms.load(std::memory_order_relaxed);
        const long long pose_age_ms = (last_ms > 0) ? (now_ms - last_ms) : -1;
        constexpr long long kPoseStaleMs = 1200;
        if (last_ms > 0 && pose_age_ms > kPoseStaleMs && (std::chrono::steady_clock::now() - last_pose_stale_log) >= std::chrono::seconds(1)) {
            std::cerr << "[POSE_STALE] age_ms=" << pose_age_ms
                      << " x=" << pose.x << " y=" << pose.y << "\n";
            last_pose_stale_log = std::chrono::steady_clock::now();
        }
    }
    ...
    constexpr double kReverseForwardYawScale = 0.45;  // forward pulse: 우회전(앞으로 이동)
    constexpr double kReverseTurnYawSign = 1.0;
    constexpr double kReverseForwardYawSign = -1.0;   // reverse에서의 방향 반전이 적용되므로 -로 우회전에 맞춤
    constexpr double kReverseForwardSpeedScale = 0.35;
    ...
    if (cmd.speed_mps < -kReverseMinAbsSpeed) {
        const double reverse_phase_speed = std::max(0.1, std::fabs(cmd.speed_mps));
        if (reverse_wiggle_state == ReverseWiggleState::NONE) {
            reverse_wiggle_state = ReverseWiggleState::REVERSE_LEFT;
            reverse_wiggle_until = now + kReverseWiggleTurnMs;
        }
        if (now >= reverse_wiggle_until) {
            if (reverse_wiggle_state == ReverseWiggleState::REVERSE_LEFT) {
                reverse_wiggle_state = ReverseWiggleState::FORWARD_PULSE;
                reverse_wiggle_until = now + kReverseForwardMs;
            } else {
                reverse_wiggle_state = ReverseWiggleState::REVERSE_LEFT;
                reverse_wiggle_until = now + kReverseWiggleTurnMs;
            }
        ...
        if (reverse_wiggle_state == ReverseWiggleState::REVERSE_LEFT) {
            cmd.speed_mps = -reverse_phase_speed;
            cmd.yaw_rate_rps = clamp(
                kReverseTurnYawSign * max_yaw_rate_rps_ * kReverseTurnYawScale,
                -max_yaw_rate_rps_,
                max_yaw_rate_rps_);
            status.mode = "REV_WIGGLE_TURN";
        } else {
            cmd.speed_mps = reverse_phase_speed * kReverseForwardSpeedScale;
            cmd.yaw_rate_rps = clamp(
                kReverseForwardYawSign * max_yaw_rate_rps_ * kReverseForwardYawScale,
                -max_yaw_rate_rps_,
                max_yaw_rate_rps_);
            status.mode = "REV_WIGGLE_PULSE";
        }
    } else {
        reverse_wiggle_state = ReverseWiggleState::NONE;
    }
    ...
    publishStatus(status);
    if (goal.valid && pose.valid && dr_initialized && !reached_hold) {
        const auto now = std::chrono::steady_clock::now();
        ...
        if (dt > 0.0 && (std::fabs(cmd.speed_mps) > 0.01 || std::fabs(cmd.yaw_rate_rps) > 0.01)) {
            ...
            static auto last_dr_log = std::chrono::steady_clock::now() - std::chrono::seconds(1);
            if (now - last_dr_log >= std::chrono::seconds(1)) {
                std::cout << "[POSE_DR] x=" << pose.x << " y=" << pose.y << " yaw=" << pose.yaw
                          << " dt=" << std::fixed << std::setprecision(3) << dt << "\n";
                last_dr_log = now;
            }
        }
    }
...
void RcControlNode::sendCommandToRc(const RcCommand& cmd, int servo_us) const {
    ...
    const auto now = std::chrono::steady_clock::now();
    if (now - g_last_servo_log >= std::chrono::seconds(1)) {
        std::cout << "[SERVO] pwm=" << pwm << " us=" << servo_us
                  << " v=" << cmd.speed_mps << " w=" << cmd.yaw_rate_rps << "\n";
        g_last_servo_log = now;
    }
}
...
bool RcControlNode::parsePoseJson(const std::string& payload, RcPose& out_pose) {
    ...
    std::string frame;
    if (extractString(payload, "frame", frame)) {
        out_pose.frame = frame;
    }
