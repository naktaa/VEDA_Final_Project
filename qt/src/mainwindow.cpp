...
MainWindow::MainWindow(const QString& userId, QWidget *parent)
    ...
    , m_runtimeConfig(loadQtRuntimeConfig())
    , m_userId(userId)
    , m_role(resolveRoleFromUserId(userId))
{
    ...
    m_overlayCalibPath = QDir(m_runtimeConfig.configDir).filePath("overlay_calib.ini");
    loadOverlayTweak();
    ...
    // 釉뚮줈占??占쎈쾭 IP占??占쎌냽
    m_mqtt->start(m_runtimeConfig.mqttHost,
                  m_runtimeConfig.mqttPort,
                  m_runtimeConfig.mqttTopicFilter);
    m_pub->start(m_runtimeConfig.mqttHost,
                 m_runtimeConfig.mqttPort,
                 m_runtimeConfig.mainPublisherClientId);
    ...
    appendLog(QString("Runtime config: %1")
                  .arg(m_runtimeConfig.sourcePath.isEmpty()
                           ? QStringLiteral("<defaults>")
                           : m_runtimeConfig.sourcePath));
    loadUiSettings();
...
{
    m_centralRtsp.clear();
    if (!m_runtimeConfig.centralRtsp.trimmed().isEmpty()) {
        m_centralRtsp[1] = m_runtimeConfig.centralRtsp.trimmed();
    }
}
...
{
    m_wardStreams.clear();
    if (!m_runtimeConfig.tankRtsp.trimmed().isEmpty()) {
        m_wardStreams["T1"] = {m_runtimeConfig.tankRtsp.trimmed(), QString()};
        ...
        appendLog("Tank stream is not configured in config/runtime.ini");
    }
}
...
bool MainWindow::tryOpenMqttDangerClip(QListWidgetItem* item)
{
    ...
    if (!m_clipPopup) {
        m_clipPopup = new ClipPopup(m_runtimeConfig.mqttHost,
                                    m_runtimeConfig.mqttPort,
                                    m_runtimeConfig.clipPublisherClientId,
                                    this);
...
void MainWindow::loadRuViewZoneConfig()
{
    ...
    if (m_runtimeConfig.ruviewZoneConfigPath.isEmpty()) {
        appendLog("RuView zone config path is empty in config/runtime.ini");
        return;
    }

    QFile file(m_runtimeConfig.ruviewZoneConfigPath);
...
{
    QDir().mkpath(QFileInfo(m_overlayCalibPath).absolutePath());
    QSettings s(m_overlayCalibPath, QSettings::IniFormat);
