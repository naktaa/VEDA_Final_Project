 ...
 * User config
 * --------------------------------------------------------- */
#ifndef CSI_UDP_TARGET_IP
#define CSI_UDP_TARGET_IP      "192.168.100.29"
#endif

#ifndef CSI_UDP_TARGET_PORT
#define CSI_UDP_TARGET_PORT    5005
#endif

/* 보드마다 다르게 바꿔서 사용
 * 예: "esp32-rx1", "esp32-rx2", "esp32-rx3"
 */
#ifndef APP_NODE_ID
#define APP_NODE_ID            "esp32-rx1"
#endif
...
static void wifi_csi_rx_cb(void *ctx, wifi_csi_info_t *data)
{
    ...
    if ((pkt.seq % 100) == 0) {
        ESP_LOGI(TAG,
                 "CSI seq=%" PRIu32 " len=%d rssi=%d ch=%d",
...
void app_main(void)
{
    ...
    ESP_LOGI(TAG, "ESP_WIFI_MODE_STA + CSI + UDP (RX)");
    wifi_init_sta();
}
