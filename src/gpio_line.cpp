...
GpioOutputLine::~GpioOutputLine() {
    close();
}

bool GpioOutputLine::open(const std::string& chip_path,
                          unsigned int offset,
    ...
    if (!settings || !line_config || !request_config) {
        set_error(error, "failed to allocate libgpiod output request state");
    ...
    if (gpiod_line_config_add_line_settings(line_config, &offset_, 1, settings) < 0) {
        set_error(error, errno_string("failed to configure GPIO output line"));
    ...
    gpiod_line_settings_free(settings);
    if (!request_) {
        set_error(error, errno_string("failed to request GPIO output line"));
        close();
        return false;
    }
#else
    ...
    if (gpiod_line_request_output(line_, consumer.c_str(), initial_value ? 1 : 0) < 0) {
        set_error(error, errno_string("failed to request GPIO output line"));
        close();
        return false;
    }
...
bool GpioOutputLine::set_value(int value, std::string* error) {
    if (!is_open()) {
        set_error(error, "GPIO output line is not open");
        return false;
    }

#ifdef USE_LIBGPIOD_V2
    if (gpiod_line_request_set_value(request_,
                                     offset_,
                                     value ? GPIOD_LINE_VALUE_ACTIVE : GPIOD_LINE_VALUE_INACTIVE) < 0) {
        set_error(error, errno_string("failed to set GPIO output value"));
        return false;
    }
#else
...
bool GpioOutputLine::is_open() const {
#ifdef USE_LIBGPIOD_V2
