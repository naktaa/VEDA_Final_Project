...
bool RcStatusPublisher::start() {
    ...
    if (!mosq_) {
        std::cerr << "[STATUS] mosquitto_new failed\n";
        return false;
    }
    ...
    if (rc != MOSQ_ERR_SUCCESS) {
        std::cerr << "[STATUS] mosquitto_will_set failed rc=" << rc
    ...
    if (rc != MOSQ_ERR_SUCCESS) {
        std::cerr << "[STATUS] mosquitto_connect failed rc=" << rc
                  << " (" << mosquitto_strerror(rc) << ")\n";
        mosquitto_destroy(mosq_);
        mosq_ = nullptr;
        return false;
    ...
    if (rc != MOSQ_ERR_SUCCESS) {
        std::cerr << "[STATUS] mosquitto_loop_start failed rc=" << rc
                  << " (" << mosquitto_strerror(rc) << ")\n";
    ...
    running_ = true;
    next_publish_at_ = std::chrono::steady_clock::now();
    std::cout << "[STATUS] publisher started broker="
...
void RcStatusPublisher::onConnect(int rc) {
    if (rc != MOSQ_ERR_SUCCESS) {
        std::cerr << "[STATUS] MQTT connect callback rc=" << rc
                  << " (" << mosquitto_strerror(rc) << ")\n";
        mqtt_connected_ = false;
        return;
    }
    ...
    std::cout << "[STATUS] MQTT connected\n";
}
...
void RcStatusPublisher::onDisconnect(int rc) {
    ...
    std::cerr << "[STATUS] MQTT disconnected rc=" << rc << "\n";
}
...
bool RcStatusPublisher::ensureConnected() {
    ...
    const auto elapsed_ms =
        std::chrono::duration_cast<std::chrono::milliseconds>(now - last_reconnect_try_).count();
    ...
    if (rc != MOSQ_ERR_SUCCESS) {
        std::cerr << "[STATUS] mosquitto_reconnect failed rc=" << rc
                  << " (" << mosquitto_strerror(rc) << ")\n";
        return false;
    }

    std::cout << "[STATUS] MQTT reconnect requested\n";
    return true;
}

bool RcStatusPublisher::publishStatusSnapshot(const RcStatus& status) {
    if (!mosq_) {
        std::cerr << "[STATUS] publish skipped: MQTT client is null\n";
        return false;
    }
    ...
    const int rc = mosquitto_publish(mosq_,
                                     ...
                                     config_.retain);
    if (rc != MOSQ_ERR_SUCCESS) {
        std::cerr << "[STATUS] publish failed rc=" << rc
                  << " (" << mosquitto_strerror(rc) << ")\n";
        return false;
    }
